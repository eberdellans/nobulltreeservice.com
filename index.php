...
<html>
    ...
    <body>
                ...
                <div class="contact-grid">
                    <div class="contact-cta">
                        <div class="contact-cta-inner">
                            <div class="contact-cta-logo">
                                <img src="elements/no-bull-tree-service-transparent-noseearring.png" alt="No Bull Tree Service logo">
                            </div>
                            <div class="contact-cta-details">
                                ...
                                <div class="social-icons" aria-label="Social media links">
                                    <a href="https://facebook.com" class="social-icon social-icon--fb" aria-label="Facebook" target="_blank" rel="noopener">
        ...
        <section id="blog">
            ...
            <script>
                // Build excerpt up to the first colon from hidden content
                (function(){
                    document.querySelectorAll('#blog article').forEach(function(article){
                        var excerptP = article.querySelector('.blog-excerpt p');
                        var content = article.querySelector('.blog-content');
                        if (!excerptP || !content) return;
                        var firstP = content.querySelector('p');
                        if (!firstP) return;
                        var txt = (firstP.textContent || '').trim();
                        var idx = txt.indexOf(':');
                        if (idx !== -1) {
                            var before = txt.slice(0, idx + 1).trim();
                            var after = txt.slice(idx + 1).trim();
                            if (before) excerptP.textContent = before;
                            if (after) firstP.textContent = after; else firstP.remove();
                        }
                    });
                })();
